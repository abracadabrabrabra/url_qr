...
def generate_qr_with_logo(url: str, logo_path: str, scale: int = 10) -> bytes:
    ...
    qr.save(temp_buffer, kind='png', scale=scale)
    temp_buffer.seek(0)

    qr_img = Image.open(temp_buffer).convert('RGB')
...
def generate_qr_with_custom_params(
    ...
    if logo_path:
        return generate_qr_with_logo(url, logo_path, scale)
    if dark_color and light_color:
        return generate_custom_color_qr(url, dark_color, light_color, scale)
    return generate_simple_qr(url, scale)